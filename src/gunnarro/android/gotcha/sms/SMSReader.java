...
public class SMSReader extends javax.mail.Authenticator {
	 ...
	 * @return
	 */
	public List<SMS> getSMSInbox(boolean isOnlyUnread) {
		List<SMS> smsInbox = new ArrayList<SMS>();
	...
	public List<SMS> getSMSOutboxGroupBy(String period, boolean isGroupByAddress) {
		...
		Map<String, SMS> smsInboxMap = new HashMap<String, SMS>();
		Uri smsUri = Uri.parse(CONTENT_SMS_INBOX);
		Cursor cursor = null;
		try {
		...
		// return smsInbox;
		return new ArrayList<SMS>(smsInboxMap.values());
	}
